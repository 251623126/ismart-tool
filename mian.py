...
def get_ut(param, param1):
    ...
    response = requests.post('http://124.223.8.252:84/ismart', headers=headers, data=data)
...
class Login(object):
            ...
            for a in name_list[course_id]['page']:
                for page in pages:
                    if a['Id'] == page['parentId']:
                        self.submit(page['parentId'], page['taskId'], task_no)
            continue
    ...
    def submit(self, parent_id, task_id, task_no):
        headers = {
            'User-Agent': 'Android-Ismart-Moblie 2.5.3',
            # 'uid': '1570807', 暂时不需要
        ...
        score = random.choice([90, 89, 92, 93, 94, 95, 85])

        tim = random.choice([60, 57, 58, 70, 80, 120])
